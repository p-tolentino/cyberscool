...
export type TerminalLoadingIconProps = DotMatrixCommonProps
...
export function TerminalLoadingIcon({
  size = 30,
  dotSize = 6.5,
  color = "currentColor",
  ariaLabel = "Loading",
      ...
      >
        {Array.from({ length: MATRIX_SIZE * MATRIX_SIZE }).map((_, index) => {
          ...
          if (isActive) {
            ...
            const head =
              Math.floor((frame / STEP_COUNT) * PERIMETER_PATH.length) %
              PERIMETER_PATH.length
            for (let trail = 0; trail < TAIL_LEVELS.length; trail += 1) {
              const idx =
                (head - trail + PERIMETER_PATH.length) % PERIMETER_PATH.length
